...
Run the Challenge 1 hex-to-Base64 converter:

```bash
cargo run --bin hex_to_base64
```

Run the Challenge 2 fixed-XOR program:
...
└── src/
    ...
    └── bin/
        ├── fixed_xor.rs
        └── hex_to_base64.rs
        └── single_byte_xor_cipher.rs
```

- `src/lib.rs` contains reusable encoding and cryptography functions.
- `src/bin/` contains the executable entry point for each challenge.
- `docs/` contains implementation-oriented challenge specifications.
